...
- [ ] Replace the in-repo `voll_mit_vox_*` audio + `shows/renders/` material with a small distributable demo show set. Local working-tree litter is already suppressed via `.gitignore`. **Progress:** reproducible demo *rigs* now ship in `demos/rigs/` - five standard archetypes (club band, mid-size band, festival mainstage, DJ/EDM, static theatre) built deterministically by `demos/generate_rigs.py` from the bundled `custom_fixtures/` defs, so they load on any machine; see `demos/README.md`. Remaining: a demo *show* (structure + a short royalty-free audio clip) layered on one of these rigs, and removal of the `voll_mit_vox_*` material.
